...
Spring Boot 4.0.3 기반의 커머스 백엔드. 현재는 단일 모놀리식 애플리케이션으로, 5개의 Bounded Context를 레이어드 아키텍처로 구현한 상태. JWT 기반 인증 적용 완료.
...
| Method | Path | 설명 | 인증 |
|---|---|---|---|
| `POST` | `/orders` | 주문 생성 (재고 차감 포함) | 🔒 JWT |
| `GET` | `/orders/{orderId}` | 주문 단건 조회 | 공개 |
| `POST` | `/orders/{orderId}/cancel` | 주문 취소 | 공개 |
...
Wallet / Seller → Settlement → 이벤트 기반 아키텍처 → CI/CD
