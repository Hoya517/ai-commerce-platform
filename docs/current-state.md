...
| Method | Path | 설명 | 인증 |
|---|---|---|---|
| `POST` | `/payments` | 결제 요청 | 공개 |
| `POST` | `/payments/confirm` | 결제 승인 (→ 주문 PAID 상태로 변경) | 공개 |
| `POST` | `/payments/fail` | 결제 실패 처리 | 공개 |
| `POST` | `/payments/wallet` | 예치금으로 결제 (즉시 APPROVED) | 🔒 JWT |
| `POST` | `/payments/{paymentId}/cancel` | 결제 취소/환불 (재고 복구, WALLET 예치금 환불) | 🔒 JWT |
...
### Settlement — `/settlements` 🔒 JWT 필요 (판매자 전용)

| Method | Path | 설명 |
|---|---|---|
| `GET` | `/settlements/me` | 내 정산 목록 조회 (당월 누적 기준) |
...
- Settlement 도메인: FeePolicy(10%), 결제 이벤트 구독 → 당월 정산 누적, 조회 API (ISSUE-18/19/21)
